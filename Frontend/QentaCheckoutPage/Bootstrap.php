...
/**
 * QentaCheckoutPage Bootstrap class
 *
 * This class is hooking into the bootstrap mechanism of Shopware.
 */
class Shopware_Plugins_Frontend_QentaCheckoutPage_Bootstrap extends Shopware_Components_Plugin_Bootstrap
{
    /**
     * @var string
     */
    const CONTROLLER = "QentaCheckoutPage";

    /**
     * Starting position for Qenta CEE payment methods
     */
     ...
     * Plugin name
     */
    const NAME = 'Shopware_5.QentaCheckoutPage';
    ...
    public function update($version) {
        if (version_compare($version, '1.0.0', '<=')) {
            ...
            Shopware()->Db()->delete('s_core_paymentmeans', 'name = "qcp_c2p"');
        }
        ...
        if (version_compare($version, '1.4.4', '<=')) {
            ...
            Shopware()->Db()->delete('s_core_paymentmeans', 'name = "qcp_quick"');
            Shopware()->Db()->delete('s_core_paymentmeans', 'name = "qcp_elv"');
            Shopware()->Db()->delete('s_core_paymentmeans', 'name = "qcp_mpass"');
            Shopware()->Db()->delete('s_core_paymentmeans', 'name = "qcp_skrilldirect"');
    ...
    protected function createForm()
    {
        ...
        $form->setElement(
            ...
            array(
                ...
                'scope' => \Shopware\Models\Config\Element::SCOPE_SHOP,
                'description' => 'URL zu einem Bild/Logo, das während des Bezahlprozesses in Qenta Checkout Page angezeigt wird (vorzugsweise 95x65 px).',
                'required' => false,
                'order' => ++$i
            )
        );
        ...
        $form->setElement(
            ...
            array(
                ...
                'value' => 0,
                'scope' => \Shopware\Models\Config\Element::SCOPE_SHOP,
                'description' => 'Benachrichtigung per E-Mail über Zahlungen Ihrer Kunden, falls ein Kommunikationsproblem zwischen Qenta und Ihrem Onlineshop aufgetreten ist.',
                'required' => false,
                'order' => ++$i
            )
        );
        ...
        $form->setElement(
            ...
            array(
                ...
                'value' => 1,
                'scope' => \Shopware\Models\Config\Element::SCOPE_SHOP,
                'description' => 'Qenta Checkout Page in iFrame anzeigen.',
                'required' => false,
                'order' => ++$i,
            )
        );
    ...
    public function createTranslations()
    {
        ...
        $translations = Array(
            'en_GB' => Array(
                'CUSTOMERID' => Array(
                    'label' => 'Customer ID',
                    'description' => 'Customer number you received from Qenta (customerId, i.e. D2#####).'
                ),
                'SHOPID' => Array(
                    'label' => 'Shop ID',
                ...
                'IMAGE_URL' => Array(
                    ...
                    'description' => 'URL to an image/logo which is displayed during the payment process in Qenta Checkout Page (95x65 px preferred)'
                ),
                'CONFIRM_MAIL' => Array(
                    'label' => 'Notification e-mail',
                    'description' => 'Receiving notification by e-mail regarding the orders of your consumers if an error occurred in the communication between Qenta and your online shop.'
                ),
                'AUTO_DEPOSIT' => Array(
        ...
        foreach (Shopware()->Container()->get('QentaCheckoutPage')->getPaymentMethods()->getList() as $pm) {
            ...
            if(!$oPayment) {
                ...
                if (isset($pm['additionalDescription']) && strlen($pm['additionalDescription']) && !is_null($pm['additionalDescription'])) {
                    ...
                    if ( $additional === '' ) {
                        if ($oPayment->getTemplate() == 'qenta_logos.tpl') {
     ...
     *
     * @return Shopware_Plugins_Frontend_QentaCheckoutPage_Models_Resources
     */
    ...
    public function afterInit()
    {
        ...
        $this->get('Loader')->registerNamespace('Shopware\\Plugins\\QentaCheckoutPage', $this->Path());
    ...
    {
        Shopware_Plugins_Frontend_QentaCheckoutPage_Bootstrap::init();
    ...
    /**
     * set confirmmail after ordercreation false (only for QentaCheckoutPage)
     * @param Enlight_Event_EventArgs $args
     * @return bool
     */
    public function defineSending(Enlight_Event_EventArgs $args)
    {
        ...
        //only prevent confirmationmail for QentaCheckoutPage payment action
        if(($paymentaction == 'QentaCheckoutPage') && $pending) {
            //save mail confirmation for later usage
            if (isset($args['mail'])) {
                Shopware()->Session()->sQentaConfirmMail = $args['mail'];
            }
            return false;
        }
    }
        ...
        switch($args->getSubject()->Request()->getActionName())
        {
            case 'shippingPayment':
                ...
                if ( ! $this->isActivePayment('invoice')) {
                    $view->sPayments = $this->hidePayment($view->sPayments, 'qcp_invoice');
                }
                if ( ! $this->isActivePayment('installment')) {
                    $view->sPayments = $this->hidePayment($view->sPayments, 'qcp_installment');
                }
            ...
            case 'confirm':
                ...
                if ((Shopware()->Container()->get('QentaCheckoutPage')->getConfig()->INVOICE_PROVIDER == 'ratepay' && $paymentName == 'qcp_invoice') ||
                    ...
                    $view->qcpConsumerDeviceIdScript = $ratepay;
                }
            ...
            case 'finish':
                ...
                $view->qcpPendingPayment = $args->getSubject()->Request()->get('ispending');
                break;
